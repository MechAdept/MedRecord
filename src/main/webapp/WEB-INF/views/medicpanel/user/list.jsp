        ...
        <div class="col-xs-4">
            ...
            <div class="btn-group">
                ...
                <ul class="dropdown-menu">
                    <li>
                        <a href="<c:url value="/medicpanel/user/patients?pageNo=${pageNo}&pageSize=${7}&desc=${desc}&sort=${sort}"/>">7</a>
                    </li>
                    <li>
                        <a href="<c:url value="/medicpanel/user/patients?pageNo=${pageNo}&pageSize=${15}&desc=${desc}&sort=${sort}"/>">15</a>
                    </li>
                    <li>
                        <a href="<c:url value="/medicpanel/user/patients?pageNo=${pageNo}&pageSize=${25}&desc=${desc}&sort=${sort}"/>">25</a>
                    </li>
                </ul>
            </div>
    ...
    <table class="table">
        ...
        <tr>
            <th scope="col">
                <c:if test="${desc == false}">
                    <a href="<c:url value="/medicpanel/user/patients?pageNo=${pageNo}&pageSize=${pageSize}&desc=true&sort=id"/>">id</a>
                </c:if>
                <c:if test="${desc == true}">
                    <a href="<c:url value="/medicpanel/user/patients?pageNo=${pageNo}&pageSize=${pageSize}&desc=false&sort=id"/>">id</a>
                </c:if>
            </th>
            <th scope="col">
                <c:if test="${desc == false}">
                    <a href="<c:url value="/medicpanel/user/patients?pageNo=${pageNo}&pageSize=${pageSize}&desc=true&sort=surname"/>"><spring:message
                            code="text.label.surname"/></a>
                </c:if>
                <c:if test="${desc == true}">
                    <a href="<c:url value="/medicpanel/user/patients?pageNo=${pageNo}&pageSize=${pageSize}&desc=false&sort=surname"/>"><spring:message
                            code="text.label.surname"/></a>
                </c:if>
            </th>
            <th scope="col">
                <c:if test="${desc == false}">
                    <a href="<c:url value="/medicpanel/user/patients?pageNo=${pageNo}&pageSize=${pageSize}&desc=true&sort=name"/>"><spring:message
                            code="text.label.name"/></a>
                </c:if>
                <c:if test="${desc == true}">
                    <a href="<c:url value="/medicpanel/user/patients?pageNo=${pageNo}&pageSize=${pageSize}&desc=false&sort=name"/>"><spring:message
                            code="text.label.name"/></a>
                </c:if>
            </th>
            <th scope="col">
        ...
        <c:forEach items="${DTOList}" var="user">
            <tr>
                ...
                <th scope="row">${user.surname}</th>
                <td><a href="<c:url value="/medicpanel/user/${user.id}"/>" class="btn-sm btn-primary"
                       role="button"
                       aria-pressed="true"><spring:message code="button.details"/></a>
                </td>
                <td><a href="<c:url value="/medicpanel/health/${user.id}"/>" class="btn-sm btn-success"
                       role="button"
                       aria-pressed="true"><spring:message code="button.health"/></a>
                </td>
                <td><a href="<c:url value="/medicpanel/ticket/${user.id}"/>" class="btn-sm btn-warning" role="button"
